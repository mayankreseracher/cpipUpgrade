...
class TracingConfig:
    ...
    def from_env(cls) -> TracingConfig:
        """Load tracing config from environment."""
        enabled = _parse_bool(os.getenv("OTEL_ENABLED", "false"))
        otlp_endpoint = os.getenv(
            "OTEL_EXPORTER_OTLP_ENDPOINT", "http://localhost:4317"
        )
        service_name = os.getenv("OTEL_SERVICE_NAME", "cpip-server")
        service_version = os.getenv("OTEL_SERVICE_VERSION", "0.1.0")
        environment = os.getenv("OTEL_ENVIRONMENT", "development")
        sample_rate = _parse_float(os.getenv("OTEL_SAMPLE_RATE", "1.0"), 1.0)

        if enabled and not otlp_endpoint:
            logger.warning(
                "OpenTelemetry enabled but OTEL_EXPORTER_OTLP_ENDPOINT not set. "
